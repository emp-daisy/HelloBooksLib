...
let userId;
const passwordToken =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiZW1haWwiOiJqb2huLmRvZUB0ZXN0LmNvbSIsImlhdCI6MTU1OTkxMDkzMX0.L8oDiM8GDx11vpv3KVjI0RLPduLCgnYq3qddgxH_ROs';
const wrongPasswordToken =
  'eyJhbGciOi.eyJpZCI6MSwiZW1haWwiOiJ.rAb7aZJip36siJGnU';
...
describe('test for verifying email', () => {
  it("Should fail if token doesn't match the user email_confirm_code token", async done => {
    server()
      .get(
        `${url}/auth/verifyemail?token=${token.substring(2, 30)}&id=${userId}`
      )
      .end((err, res) => {
        expect(res.statusCode).toEqual(401);
        expect(res.body).toHaveProperty('error');
  ...
  it('Should create a new User in the db', async done => {
    server()
  ...
  it('Should not create a new User in the db if the user exists', async done => {
    server()
      .post(`${url}/auth/users/user`)
      .set('authorization', `Bearer ${superAdminToken}`)
      .send(users[3])
